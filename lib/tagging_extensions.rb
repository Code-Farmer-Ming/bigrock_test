class ActiveRecord::Base #:nodoc:
  ...
  module TaggingExtensions
    ...
    def _remove_tags outgoing
      ...
      outgoing_tags = tags.find_all_by_name(outgoing)
      outgoing_taggings = taggings.find_all_by_tag_id(outgoing_tags.map(&:id))
  ...
  module TaggingFinders
    ...
    def parse_tags(i_tags)
      return [] if i_tags.blank?
      i_tags = Array(i_tags).first
      i_tags = i_tags.respond_to?(:flatten) ? i_tags.flatten : i_tags.split(Tag::DELIMITER)
      i_tags.map { |i_tags| i_tags.strip.squeeze(" ") }.flatten.compact.map(&:downcase).uniq
    end
