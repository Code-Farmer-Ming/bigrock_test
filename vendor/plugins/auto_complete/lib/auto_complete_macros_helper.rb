module AutoCompleteMacrosHelper      
  ...
  def auto_complete_field(field_id, options = {})
    ...
    if options[:active]
      options[:active].each do |k,v|
        function << "Event.observe($('#{k}'), '#{v}',#{field_id}_auto_completer.activate.bind(#{field_id}_auto_completer));"
      end
  ...
  def auto_complete_result(entries, field, phrase = false)
