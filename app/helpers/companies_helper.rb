module CompaniesHelper
  ...
  def options_for_industry(default_id)
    options_for_select([['所有行业',0]]+Industry.select_options(),:selected=>default_id)
  end
  
  def options_for_company_type(default_id)
    options_for_select([['所有类型',0]]+CompanyType.select_options,default_id)
  end

  def options_for_company_size(default_id)
    options_for_select([['所有规模',0]]+CompanySize.select_options,default_id)
  end

  def options_for_state(default_state)
    options_for_select([['所有省',0]]+State.all.collect {|p| [p.name,p.id]},default_state)
  end
