...
class TaggingTest < ActiveSupport::TestCase
  fixtures :tags, :taggings, :companies
  ...
  def test_find_tagged_with
    ...
    assert_equal Company.tagged_with("ipa lager"), result1
    assert_equal Company.tagged_with("ipa", "lager"), result1
    
    result2 = [@obj1.id, @obj2.id].sort
    assert_equal Company.tagged_with("seasonal").map(&:id).sort, result2
    assert_equal Company.tagged_with("seasonal lager").map(&:id).sort, result2
    assert_equal Company.tagged_with("seasonal", "lager").map(&:id).sort, result2
  end
